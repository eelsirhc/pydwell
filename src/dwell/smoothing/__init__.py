...
def smooth(data, weights, return_weights=False, allow_edge_effects=False):
    """Smooth 1-dimensional data using numpy.convolve based on the weights.
    ...
    :param return_weights: whether to include the weights and the output
...
def scipy_window_smooth(data, boxsize=9,
                        window_name='triangle',
                        options=None, *args, **kwargs):
    """
    Use a scipy defined window to smooth.

    Options include:
    'triangle','boxcar','gaussian','blackman','parzen','hamming'.

    All but the 'gaussian' option takes a single option of
    the window size. 'gaussian' also takes the width.
...
def box(data, boxsize=9, *args, **kwargs):
    """Smooth with a uniform distribution.

    Generates an array of length N filled with values of 1/N,
    then calls smooth with this window
...
def triangle(data, boxsize=9, *args, **kwargs):
    """Smooth with a triangular distribution.
    ...
    :return: Results from smooth, which is either the smoothed data,
    or smoothed data and weights used in the smooothing
    :rtype: numpy array
    """
    boxsize = oddbox("box", boxsize, *args, **kwargs)
...
def gaussian(data, boxsize=9, nsigma=3, *args, **kwargs):
    ...
    Size boxsize that covers nsigma widths.
    Generates an array of length N filled with
    values of 1/N, then calls smooth with this window
    ...
    :param nsigma: number of sigmas (std devs) to contain in the
    len(boxsize) elements.
    :type nsigma: float

    Larger values of sigma give a tighter distribution in the centre and more
     delta-function like behaviour, low sigma value gives a smoother
     distribution with more uniform like
    behaviour.
