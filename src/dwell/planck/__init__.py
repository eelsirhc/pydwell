...
def cfst4_wavelength(wl, wh, temperature,
    ...
    for i in range(2):
        ...
        for m in range(1, 1000):
            wv = float(m) * v
            ff = (float(m) ** (-4)) *\
                 (np.exp(-wv)) * (((wv + 3.0) * wv + 6.0) * wv + 6.0)
            f[i] = f[i] + ff
            if f[i] == 0.0:
                continue
            if ff / f[i] <= fac:
...
def cfst4_wavelength_weave(wl, wh, temperature,
                           diff_planck=None, diff_planck_2=None):
    """Calculate the integrated Planck function between two wavenumbers.

    Uses blitz.weave code to speed up the calculation.
    ...
    err = weave.inline(code,
                       ['f', 'temperature', 'wn', 'fac', 'sigma', 'c', 'c2'],
                       type_converters=converters.blitz, compiler='gcc')

    return err


def cfst4_wavelength_weave_loop(wl, wh, temperature,
                                diff_planck=None, diff_planck_2=None):
    """Calculate the integrated Planck function between two wavelengths.
...
def st4(temp):
    """Calculate the integrated Black body function from zero to infinity.
